...
public class SwerveSetpointGenerator {
    ...
    public SwerveSetpoint generateSetpoint(
        ...
        for (int m = 0; m < config.numModules; m++) {
            ...
            if (all_modules_should_flip) {
                double required_rotation_rad = Math.abs(
                    prev_heading[m].unaryMinus().rotateBy(desired_heading[m]).getRadians()
                );
        ...
        double dtheta =
            desiredStateRobotRelative.omegaRadiansPerSecond - prevSetpoint.robotRelativeSpeeds().omegaRadiansPerSecond;
        ...
        for (int m = 0; m < config.numModules; m++) {
            ...
            if (epsilonEquals(prevSetpoint.moduleStates()[m].speedMetersPerSecond, 0.0)) {
                ...
                var necessaryRotation = prevSetpoint
                    .moduleStates()[m].angle.unaryMinus()
            ...
            double maxHeadingChange =
                ...
                ((config.massKG / config.numModules) * Math.abs(prevSetpoint.moduleStates()[m].speedMetersPerSecond));
        ...
        ChassisSpeeds chassisAccel = new ChassisSpeeds(
            chassisAccelVec.getX(),
            chassisAccelVec.getY(),
            chassisAngularAccel
        );
    ...
    private static boolean epsilonEquals(ChassisSpeeds s1, ChassisSpeeds s2) {
        return (
            epsilonEquals(s1.vxMetersPerSecond, s2.vxMetersPerSecond) &&
